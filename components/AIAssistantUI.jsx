...
export default function AIAssistantUI() {
  ...
  // --- NEW: remove unwanted standalone labels from the DOM if present ---
  useEffect(() => {
    const unwanted = new Set(["Certified", "Personalized", "Experienced", "Helpful"])
    // find simple text nodes inside elements and remove their parent element if exact match
    const nodes = Array.from(document.querySelectorAll("body *"))
    for (const el of nodes) {
      // skip elements with children (we want standalone text labels)
    ...
    // also try to remove nodes that contain only the word inside spans
    // (this is extra-safe for different markup)
  ...
  function handleUseTemplate(template) {
    if (composerRef.current && composerRef.current.insertTemplate) {
